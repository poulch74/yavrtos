 ...
 *
 * The function could not be performed because the task making the request was not valid (eg a lower-priority
 * task trying to stop a higher-priority task).
 */
#define YAVRTOS_INVALID_TASK 1
/**
 * \ingroup returnvalues
 * \brief Invalid function for idle task
 *
...
/**
 * \ingroup returnvalues
 * \brief Invalid function for an ISR
 *
...
/**
 * \ingroup returnvalues
 * \brief RTOS not started
 */
#define YAVRTOS_RTOS_NOT_STARTED 6
/**
 * \ingroup returnvalues
 * \brief Mailbox not empty
 */
#define YAVRTOS_MBOX_NOT_EMPTY 7
/**
 * \ingroup returnvalues
 * \brief The task was already stopped
 */
...
typedef struct taskstruct {
	...
	/// \internal The semaphore we use to time out while waiting for other semaphores
	semaphore_t *timeout_semaphore;
	/// \internal The value of the timeout semaphore to wait for
	int16_t timeout_semaphore_min_value;
	/**
 ...
 * \li #YAVRTOS_OK
 * \li #YAVRTOS_INVALID_TASK
 * \li #YAVRTOS_IDLE_TASK
 * \li #YAVRTOS_ALREADY_STOPPED
 *
 ...
 *			</ul>
 * <li> \c timeout_semaphore - the semaphore to use for timing out while waiting, or zero if no timeout should occur
 * <li> \c timeout - the amount the timeout semaphore should increment by before a timeout occurs.
 * </ul>
 *
 * The possible return values are
 * \li #YAVRTOS_OK
 * \li #YAVRTOS_ISR
 * \li #YAVRTOS_IDLE_TASK
 * \li #YAVRTOS_RTOS_NOT_STARTED
 * \li #YAVRTOS_TIMEOUT
 *
 * Since this function can cause a task suspension, it can only be called from a task with a non-zero priority
 */
yavrtos_result_t write_mbox(mailbox_t *mbox, void *data, uint8_t wait_for_receivers, uint8_t wait_for_empty_nullify,
	semaphore_t *timeout_semaphore, int16_t timeout);
 ...
 * \arg \c value - the minimum value the semaphore should reach
 ...
 * \arg \c timeout - the amount that the timeout semaphore should increment before timeout occurs. Note that the value
 *          of this argument should not exceed about 32,000, as semaphores use signed 16-bit numbers
 *
 ...
 * \ingroup semaphore
 *
 * \brief Wait for a semaphore to reach at least a particular value, but with a set value for the timeout, as opposed to
 *        an increment to the timeout semaphore value.
 *
 * The arguments are
 * \arg \c p - the semaphore to wait on
 * \arg \c value - the minimum value the semaphore should reach
 * \arg \c timeout_semaphore - a semaphore to use for timing out while waiting on the other semaphore, or 0 if no
 *          timeout should be performed
 * \arg \c timeout_value - the minimum value the timeout semaphore should reach before triggering a timeout
 *
 * Since the calling task could be suspended, this can only be called by a task with a non-zero priority
 *
 * The possible return values are
 ...
 * \arg \c timeout_semaphore - a semaphore to use for timing out while waiting on the other semaphore, or 0 if no
 *          timeout should be performed
 * \arg \c timeout - the amount that the timeout semaphore should increment before timeout occurs. Note that there is
 *          a similar restriction on the value of this argument - it should not exceed about 32,000
 *
 * Since the calling task could be suspended, this can only be called by a task with a non-zero priority
 *
 * The possible return values are
 ...
 * \ingroup semaphore
 *
 * \brief Wait for a semaphore to increment its value by a certain amount, but with a set value for the timeout, as opposed to
 *        an increment to the timeout semaphore value.
 *
 * The arguments are
 * \arg \c p - the semaphore to wait on
 ...
 * executed
 */
extern struct yavrtos_system_struct yavrtos_system;
 ...
 * 	// Now, we have either interrupted a task or interrupted another ISR. yavrtos_system.interrupted_task will
 * 	// be non-zero if a task has already been interrupted - i.e. if we have interrupted an ISR
 * 	// Note that task_switch() also sets yavrtos_system.interrupted_task - any ISRs that manage to run during
 * 	// the brief period when task_switch() enables interrupts should not subsequently run task_switch()!
 * 	if (yavrtos_system.interrupted_task) { // We have interrupted an ISR (or we have interrupted task_switch())
 * 		// Execute the macro to see if we should do a task switch
 * 		if (do_task_switch) {
 * 			// The macro may have enabled interrupts - disable them again
 * 			cli();
 * 			// Set bit 1 of yavrtos_system.interrupted_task - the bit that signals that a task switch is
 ...
 * 			// If we have actually interrupted task_switch(), then setting bit 1 of yavrtos_system.interrupted_task
 * 			// will have no effect - but we were doing a task switch anyway!
 * 			yavrtos_system.interrupted_task = 3;
 * 		} else {
 ...
 * 		// Set the yavrtos_system.interrupted_task so that subsequent ISRs will know that they have interrupted
 * 		// an ISR, not a task. This has the side-effect of disabling switch_task() - all API calls check
 * 		// yavrtos_system.interrupted_task and do NOT perform a task switch if it is non-zero.
 ...
 * 			cli();
 * 			// Signal to ourselves that we need a task switch - bit 1 of yavrtos_system.interrupted_task is set
 ...
 * 		if (yavrtos_system.interrupted_task & 2) {
 * 			// switch_task() will (eventually) reset yavrtos_system.interrupted_task, and never "returns"
 ...
 * 		// We are about to return to the task - reset yavrtos_system.interrupted_task
...
void vector(void) { \
	save_cpu_context(); \
	*(((uint8_t*)SP)+1) |= _BV(SREG_I); \
	if (yavrtos_system.interrupted_task) { \
		if (do_task_switch) { \
			cli(); \
			yavrtos_system.interrupted_task = 3; \
		} else { \
